# Copyright 2025 Ali Ns
# @author: Ali Ns <aliimrandtb@gmail.com>
...
class IrSequence(models.Model):
    ...
    def to_roman(self, number):
        ...
        if not (0 <= number < 5000):
            raise UserError("number out of range (must be 0..4999)")
    ...
    def _check_padding(self):
        for rec in self:
            if rec.is_roman and rec.padding != 1:
                raise UserError('To use roman numbering, sequence size must be equal to 1')
            
    def get_next_char(self, number_next):
        #number_next is (15,)
        #ensure number_next is integer
        if self.is_roman:
